...
Socket::Socket(const std::string& host, int port, int max_conn) {
    _create_socket();
    _configure_socket();
    _set_non_blocking();
...
void Socket::_create_socket() {
    ...
    /*
        - AF_INET indica que o socket usará endereços IPv4 
            para comunicação (AF_INET6: Para endereços IPv6).
        - SOCK_STREAM indica que o socket será do tipo orientado 
            a conexão, ou seja, usará o protocolo TCP.
        - Quando o valor é 0, o sistema operacional escolhe 
            automaticamente o protocolo padrão para o tipo de 
            socket especificado. No caso de SOCK_STREAM, o 
            protocolo padrão é o TCP.
    */
...
void Socket::_configure_socket() {
    ...
    /*
        - SOL_SOCKET: Indica que a opção que está sendo configurada 
            é uma opção de nível de socket.
        - SO_REUSEADDR: Especifica a opção que está sendo configurada. 
            SO_REUSEADDR permite que o endereço do socket seja reutilizado, 
            o que é útil para evitar erros como "Address already in use" 
            quando você tenta reiniciar um servidor rapidamente.
        - &optval: Um ponteiro para o valor da opção, que é 1 (habilitado).
        - sizeof(optval): O tamanho do valor da opção (int)
    */
}
...
// PREPARA O SOCKET PARA OUVIR NOVAS SOLICITAÇÕES
