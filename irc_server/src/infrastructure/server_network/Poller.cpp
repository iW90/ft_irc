...
void Poller::addClient(int socketFD, short events) {
...
void Poller::removeClient(int socketFD) {
...
size_t Poller::getClientCount() const {
    return fds.size();
}

struct epoll_event Poller::getClient(size_t index) const {
    if (index >= fds.size()) {
